...
namespace Mediapipe.Unity
{
    public class WebCamSource : ImageSource
    {
        ...
        private IEnumerator Initialize()
        {
            ...
            if (availableSources != null && availableSources.Length > 0)
            {
                ...
                foreach (var device in availableSources)
                {
                    ...
                    {
                        webCamDevice = device;
                        Debug.Log($"Selected front-facing camera: {device.name}");
                        yield break;
                    }
                }
                // Fallback to first camera if no front-facing camera is found
                Debug.LogWarning("No front-facing camera found, falling back to default camera");
        ...
        // New method to explicitly select the front-facing camera
        public void SelectFrontFacingCamera()
        {
            availableSources = WebCamTexture.devices;
            if (availableSources == null || availableSources.Length == 0)
            {
            ...
            Debug.LogWarning("No front-facing camera found, keeping current selection");
        }
